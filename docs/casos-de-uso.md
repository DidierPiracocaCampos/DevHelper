...
| CU-13 | Aceptar Términos y Condiciones       |  Importante   |
...
### CU-13 — Aceptar Términos y Condiciones

![Prioridad](https://img.shields.io/badge/Prioridad-Importante-666666?style=flat-square)

- **Descripcion:** el usuario acepta la versión vigente de los Términos y Condiciones. La aceptación se persiste como un snapshot versionado por usuario y se re-pide de forma no-bloqueante cuando aparece una versión nueva.
- **Actores:** Desarrollador usuario.
- **Precondiciones:** cuenta activa; sesión abierta.
- **Flujo principal (alta, source=`'register'`):**
  1. El usuario llega al formulario de registro.
  2. El checkbox `acceptTerms` exige aceptación de los Términos y Condiciones (con link a `/legal/terms` en nueva pestaña).
  3. El usuario marca el checkbox, completa el registro.
  4. Tras crear la cuenta, el sistema escribe `users/{uid}/legal/termsAccepted` con `{ version: TERMS_VERSION, lang: 'es', acceptedAt: serverTimestamp(), source: 'register' }`. La escritura es no-bloqueante: un fallo se loguea con `console.warn` y no impide el registro.
- **Flujo principal (re-aceptación en modal, source=`'re-accept-modal'`):**
  1. Al iniciar la app con un usuario cuya `version` persistida es distinta a `TERMS_VERSION` (o sin snapshot), el modal `landing-terms-modal` se abre automáticamente.
  2. El usuario ve el aviso "Términos y Condiciones actualizados" y 2 botones: "Revisar" (abre `/legal/terms` en nueva pestaña) y "Aceptar y continuar" (escribe el snapshot con `source: 're-accept-modal'` y cierra el modal). No hay opción de posponer: si el usuario cierra el modal sin aceptar (vía backdrop o recarga de página), el modal vuelve a abrirse en el siguiente render hasta que se registre la aceptación.
- **Flujo principal (aceptación manual, source=`'manual'`):**
  1. El usuario navega a `/legal/terms` y revisa el documento.
  2. Si está logueado y su snapshot está desactualizado, ve un botón "Aceptar" en el header sticky.
  3. Pulsa "Aceptar" y se escribe el snapshot con `source: 'manual'`.
- **Flujos alternativos:**
  - Usuario deslogueado: el modal nunca aparece. La página `/legal/terms` es pública.
  - Versión persistida igual a `TERMS_VERSION`: el modal no se abre; el botón "Aceptar" está oculto.
- **Postcondiciones:** `users/{uid}/legal/termsAccepted` refleja la última versión aceptada con su origen.
- **Excepciones:** si la escritura del snapshot falla, la app sigue funcionando; el modal puede volver a salir en la siguiente carga.
...
- **Términos y Condiciones (CU-13):** la versión actual del documento se declara en `src/app/landing/data/terms-version.ts` (`TERMS_VERSION = '2026-07-18'`, `TERMS_LANG = 'es'`). El snapshot persistido en `users/{uid}/legal/termsAccepted` registra `{ version, lang, acceptedAt, source }`. `source` puede ser `'register'` (en el flujo de alta), `'re-accept-modal'` (modal no-bloqueante al detectar versión nueva en sesión), o `'manual'` (botón "Aceptar" en la página `/legal/terms`). Cuando `TERMS_VERSION` se incrementa, el modal `landing-terms-modal` aparece automáticamente para usuarios con versión persistida más antigua. El modal permanece hasta que el usuario acepta (escribiendo el snapshot con `source: 're-accept-modal'`); no hay opción de posponer. El uso de la app no se bloquea (el modal es informativo), pero el modal reaparece en cada carga hasta que se registra la aceptación.
