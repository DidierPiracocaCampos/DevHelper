...
- **Flujo principal (re-aceptación en modal, source=`'re-accept-modal'`):**
  1. Al iniciar la app con un usuario cuya `version` persistida es distinta a `TERMS_VERSION`, el modal `landing-terms-modal` se abre automáticamente.
  2. El usuario ve el aviso "Términos y Condiciones actualizados" y 3 botones: "Revisar" (abre `/legal/terms` en nueva pestaña), "Aceptar y continuar" (escribe el snapshot con `source: 're-accept-modal'` y cierra el modal), "Más tarde" (setea `dismissedThisSession = true` y cierra el modal; vuelve a salir al recargar o cambiar de usuario).
...
- **Términos y Condiciones (CU-13):** la versión actual del documento se declara en `src/app/landing/data/terms-version.ts` (`TERMS_VERSION = '2026-07-18'`, `TERMS_LANG = 'es'`). El snapshot persistido en `users/{uid}/legal/termsAccepted` registra `{ version, lang, acceptedAt, source }`. `source` puede ser `'register'` (en el flujo de alta), `'re-accept-modal'` (modal no-bloqueante al detectar versión nueva en sesión), o `'manual'` (botón "Aceptar" en la página `/legal/terms`). Cuando `TERMS_VERSION` se incrementa, el modal `landing-terms-modal` aparece automáticamente para usuarios con versión persistida más antigua, sin bloquear el uso de la app.
